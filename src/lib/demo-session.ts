...
export async function demoApiRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (path === "/ats-analyses" && method === "POST") {
    const analysis = { id: id("demo-ats"), user_id: DEMO_USER_ID, resume_version_id: body.resume_version_id, job_description_id: body.job_description_id, status: "completed", overall_score: 78, score_breakdown: { matched_terms: ["TypeScript", "Python"], missing_terms: ["Docker"], total_terms: 3 }, summary: { method: "Demo keyword coverage", matched: 2, missing: 1, total: 3, missing_terms: ["Docker"], disclaimer: "Demo result only; not a hiring prediction." }, created_at: now() };
