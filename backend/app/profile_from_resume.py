...
from app.documents import extract_sections, extract_skill_candidates
...
def _estimate_years(experience_entries: list[dict[str, Any]], text: str) -> float | None:
    years = sorted({int(y) for y in _YEAR_RE.findall(text or "") if 1990 <= int(y) <= 2035})
    if len(years) >= 2:
    ...
    if experience_entries:
        ...
        return float(min(len(experience_entries) * 1.5, 30))
    return None


def _infer_career_level(years: float | None) -> str | None:
    if years is None:
        return None
    if years <= 0:
        return "fresher"
...
def build_profile_draft(
    ...
    for line in header_pool + summary_lines:
        if _EMAIL_RE.search(line) or _URL_RE.search(line):
            continue
        # "City, Country" style
        if re.search(r"\b(pune|bengaluru|bangalore|hyderabad|mumbai|delhi|chennai|remote|india|usa|uk)\b", line, re.I):
            if not _looks_like_name(line) or "," in line:
                location = _clean(line, 160)
                break
    ...
    for line in skill_lines:
        ...
        # If labeled "Languages: Python, SQL"
        if ":" in line and len(chunks) <= 2:
    ...
    career_level = _infer_career_level(years)
